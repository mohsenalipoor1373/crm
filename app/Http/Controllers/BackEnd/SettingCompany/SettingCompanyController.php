...
namespace App\Http\Controllers\BackEnd\SettingCompany;
    ...
    public function index(Request $request)
    {
       ...
       <table id='data-table' class='table table-bordered table-striped text-center data-table' style='width:100%'>
        ...
        foreach ($data as $item) {
            if ($item->home_logo) {
                $home_logo = asset('/public/'.$item->home_logo);
            } else {
                $home_logo = "";
            }
            if ($item->logo_reports) {
                $logo_reports = asset('/public/'.$item->logo_reports);
            } else {
                $logo_reports = "";
            }
            $output .= "
        ...
        if ($request->ajax()) {
            ...
            return view('FrontEnd.SettingCompany.index', compact('output'));
        }
    }
