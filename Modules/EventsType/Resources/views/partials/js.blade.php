<script type="text/javascript">
    ...
    $(function () {
        ...
        $('body').on('click', '.add_events_type', function () {
        ...
        $('body').on('click', '.btn_add_events_type', function (e) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.status == 1) {
                        load_table();
                        $('#form_add_events_type').trigger("reset");
                        $('#add-events_type').modal('hide');
                        tata.success('موفق', data.success, {
                            position: 'bl',
                            duration: 8000,
                            animate: 'slide'
                        });
                        $('.btn_add_events_type').prop('disabled', false);
                        var loadingText = 'ثبت';
                        $('.btn_add_events_type').html(loadingText);
                    } else {
                        var msg;
        ...
        $('body').on('click', '.btn_edit_events_type', function (e) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.status == 1) {
                        load_table();
                        $('#form_edit_events_type').trigger("reset");
                        $('#edit-events_type').modal('hide');
                        tata.success('موفق', data.success, {
                            position: 'bl',
                            duration: 8000,
                            animate: 'slide'
                        });
                        $('.btn_edit_events_type').prop('disabled', false);
                        var loadingText = 'ثبت';
                        $('.btn_edit_events_type').html(loadingText);
                    } else {
                        var msg;
        ...
        $('body').on('click', '.edit_events_type', function () {
            $('#load_modal').show();
            $('#edit-events_type').modal('show');
            var id = $(this).data('id');
            $.ajax({
                url: "{{route('edit_events_type')}}",
                data: {id},
                type: 'GET',
                success: function (data) {
        ...
        $('body').on('click', '.remove_events_type', function () {
            var id = $(this).data('id');
            Swal.fire({
                title: 'اخطار!',
                text: "آیا از حذف رویداد اطمینان دارید؟",
                icon: 'warning',
        ...
        $('#basic_li_events_type').addClass("active");
